    ...
    useEffect(() => {
        const fetchPopularGames = async () => {
            ...
            try {
                const allGames = [];
                let currentPage = 1;
                const pageSize = 30; // Fetch 30 games per page to reach 150 in 5 pages
                ...
                while (allGames.length < 150 && currentPage <= 5) {
                    const response = await axios.get(API_URL, {
                        params: {
                            ...
                            page: currentPage,
                            page_size: pageSize,
                            ordering: "-rating", // Changed back to "-rating" as "-popularity" isn't valid
                        },
                    });
                        ...
                        <AnimatePresence>
                            {popularGames.map((game) => (
                                <motion.div
                                    ...
                                    className="bg-gray-800/80 backdrop-blur-sm rounded-xl overflow-hidden border border-indigo-500/20 shadow-xl transform-gpu"
                                >
